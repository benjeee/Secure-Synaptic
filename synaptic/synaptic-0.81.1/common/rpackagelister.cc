...
struct riskSortFuncAsc {
  /*
    mode to sort by risk
  */
  bool operator () (RPackage *x, RPackage *y){
       return (x->risk() > y->risk());
  }
};

struct riskSortFuncDes {
  /*
    mode to sort by risk in des order,
    adjusted so no-data appears at the bottom
  */
  bool operator () (RPackage *x, RPackage *y){
     if(y->risk() < 0 && x->risk() < 0)
       return (x->risk() > y->risk());
     else if(y->risk() < 0)
       return true;
     else if(x->risk() < 0)
...
struct currRiskSortFuncAsc {
   /*
   mode to sort by current risk
   */
  bool operator () (RPackage *x, RPackage *y){
    RVInfo px, py;
    double rx, ry;
    if(x->installedVersion() != NULL){
      try {
        px = pkgMap.at(x->name());
        rx = px.getCurrRisk(x->installedVersion());
      } catch (const std::out_of_range& oor) {
        rx = -1;
      }
    } else { rx = -2; }
    if(y->installedVersion() != NULL){
      try {
        py = pkgMap.at(y->name());
        ry = py.getCurrRisk(y->installedVersion());
      } catch (const std::out_of_range& oor) {
        ry = -1;
      }
    } else { ry = -2; }
...
struct riskChangeSortFuncAsc {
   /*
   mode to sort by change in risk by updating package
   */
  bool operator () (RPackage *x, RPackage *y){
    RVInfo px, py;
    double rx, ry;
    if(x->installedVersion() != NULL){
      try {
        px = pkgMap.at(x->name());
        rx = px.getCurrRisk(x->installedVersion()) - px.getUpdatedRisk();
      } catch (const std::out_of_range& oor) {
        rx = -21;
      }
    } else { rx = -22; }
    if(y->installedVersion() != NULL){
      try {
        py = pkgMap.at(y->name());
        ry = py.getCurrRisk(y->installedVersion()) - py.getUpdatedRisk();
      } catch (const std::out_of_range& oor) {
        ry = -21;
      }
    } else { ry = -22; }
				  ...
				  listSortMode mode)
{
   ...
   case LIST_SORT_INST_VERSION_DES:
      ...
      break;
      /* risk sorters */
   case LIST_SORT_RISK_ASC:
     stable_sort(packages.begin(), packages.end(),
		 sortFunc<riskSortFuncAsc>(true));
     break;
   case LIST_SORT_RISK_DES:
     stable_sort(packages.begin(), packages.end(),
		 sortFunc<riskSortFuncDes>(true));
     break;
   case LIST_SORT_CURR_RISK_ASC:
     stable_sort(packages.begin(), packages.end(),
		 sortFunc<currRiskSortFuncAsc>(true));
     break;
   case LIST_SORT_CURR_RISK_DES:
     stable_sort(packages.begin(), packages.end(),
		 sortFunc<currRiskSortFuncDes>(true));
     break;
   case LIST_SORT_RISK_CHANGE_ASC:
     stable_sort(packages.begin(), packages.end(),
		 sortFunc<riskChangeSortFuncAsc>(true));
     break;
   case LIST_SORT_RISK_CHANGE_DES:
     stable_sort(packages.begin(), packages.end(),
		 sortFunc<riskChangeSortFuncDes>(true));
     break;
   }
}
