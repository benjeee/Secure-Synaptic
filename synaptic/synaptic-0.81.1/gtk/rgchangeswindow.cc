...
{
   // new tree store
   _treeStore = gtk_tree_store_new(N_COLUMNS, G_TYPE_STRING, GDK_TYPE_PIXBUF);
   ...
   GtkCellRenderer *risk_renderer = gtk_cell_renderer_pixbuf_new();
   ...
   GtkTreeViewColumn *risk_column;
   risk_column = gtk_tree_view_column_new_with_attributes(_("Risk Score"),
                                                     risk_renderer,
                                                     "pixbuf", RISK_COLUMN, NULL);
   /* Add the columns to the view. */
   gtk_tree_view_append_column(GTK_TREE_VIEW(_tree), column);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_tree), risk_column);
			      ...
			      vector<RPackage *> &notAuthenticated)
{
  ...
  if (notAuthenticated.size() > 0) {
      ...
      for (vector<RPackage *>::const_iterator p = notAuthenticated.begin();
         ...
         risk = RGPackageStatus::pkgStatus.getRisk(*p);
