...
void RGPkgDetailsWindow::createDepRiskTable(const char *widget_name,
				    	    ...
				    	    GtkBuilder* builder)
{
   ...
   if(gtk_tree_view_get_column(GTK_TREE_VIEW(widget), 1) == NULL) {
      // cell renderer
      renderer = gtk_cell_renderer_pixbuf_new();
      column = gtk_tree_view_column_new_with_attributes("Risk",
							renderer,
							"pixbuf", 1, 
							NULL);
      gtk_tree_view_append_column(GTK_TREE_VIEW(widget), column);
   ...
   for(unsigned int i=0;i<deps.size();i++) {
	 ...
	 if(deps[i].version != NULL) {
      ...
      risk = RGPackageStatus::pkgStatus.getRiskFromName(deps[i].name);
      gtk_list_store_append(store, &iter);
      gtk_list_store_set(store, &iter, 0, depStr.c_str(),
      	      			       1, risk,
      	      			       -1);
   }
				      ...
				      bool setTitle)
{
   ...
   //this portion will be used to add the vulnerabilities. 
   //Must confirm label name
   /*
     me->setLabel("current_vulnerabilities", pkg->currVul());
     me->setLabel("past_vulnerabilities", pkg->pastVul());
     me->setLabel("curren_ risk", pkg->currRisk());
     me->setLabel("updated_risk", pkg->updatedRisk());
    */
   ...
   GtkTextIter sec_it, sec_start, sec_end;
   GtkWidget *sec_textview;
   GtkTextBuffer *sec_buf;
   const gchar *sec;

   // set security tab
   sec_textview = GTK_WIDGET(gtk_builder_get_object
                         (me->getGtkBuilder(), "textview_sec"));
   assert(sec_textview);
   sec_buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(sec_textview));
   // clear old buffer
   gtk_text_buffer_get_start_iter(sec_buf, &sec_start);
   gtk_text_buffer_get_end_iter(sec_buf, &sec_end);
   gtk_text_buffer_delete(sec_buf, &sec_start, &sec_end);
   // set security

   std::string secstr;
   try{
      RVInfo rvInfo = pkgMap.at(pkg->name());
      vector<RVulnerability> vulns = rvInfo.getVulns();
      vector<RPatch> patches = rvInfo.getPatches();
      secstr = rvInfo.getCurrVulnOutput() + rvInfo.toBePatched(pkg->installedVersion()) + rvInfo.getPatchVulnOutput();
   }catch (const std::out_of_range& oor){
     secstr = "No Vulnerability Information\n";

   }


 
   sec = utf8(secstr.c_str()); // TODO  format correctly?
   gtk_text_buffer_get_start_iter(sec_buf, &sec_it);
   gtk_text_buffer_insert(sec_buf, &sec_it, sec, -1);
   gtk_text_buffer_get_start_iter(sec_buf, &sec_start);
   ...
   // ^Old line right here, new code below gets all the risk data
   createDepRiskTable("treeview_deplist", deps, me->getGtkBuilder());
   
   // canidateVersion
   deps = pkg->enumDeps(true);
   createDepRiskTable("treeview_availdep_list", deps, me->getGtkBuilder());
   ...
   // dependencies / risk
   //deps = pkg->enumDeps();
   //me->setTreeList("dep_risk_treeview", formatDepInformation(deps), true);
   /*GtkWidget *widget = GTK_WIDGET (gtk_builder_get_object (me->getGtkBuilder(), "dep_risk_treeview"));
   if(widget == NULL)
   	   g_warning("problem here");*/
   /* TODO -- set up the dependency / risk section.
      widget used is dep_risk_treeview in window_details.ui.
      Need to figure out how bars are going to be displayed.
   */ 
   
   // ^^^ This entire commented block I couldn't get to work. I don't get it.
