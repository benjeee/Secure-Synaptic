...
static void gtk_pkg_list_init(GtkPkgList *pkg_list)
{
   ...
   pkg_list->column_headers[9] = GDK_TYPE_PIXBUF;
   pkg_list->column_headers[10] = GDK_TYPE_PIXBUF;
   pkg_list->column_headers[11] = GDK_TYPE_PIXBUF;
   pkg_list->column_headers[12] = G_TYPE_STRING;
   pkg_list->column_headers[13] = GDK_TYPE_COLOR;
   pkg_list->column_headers[14] = G_TYPE_POINTER;
}
      ...
      case RISK_COLUMN:
       {
       	 if (pkg == NULL)
             return;
          GdkPixbuf *risk;
          risk = RGPackageStatus::pkgStatus.getRisk(pkg);
          g_value_set_object(value, risk);
          break;
       }
      case CURR_RISK_COLUMN:
       {
       	  if (pkg == NULL)
             return;
          if (pkg->installedVersion() == NULL)
     	     return;
          GdkPixbuf *risk;
          risk = RGPackageStatus::pkgStatus.getCurrRisk(pkg);
          g_value_set_object(value, risk);
          break;
       }
      case UP_RISK_COLUMN:
       {
          ...
          if (pkg->installedVersion() == NULL)
     	     return;
          GdkPixbuf *risk;
          risk = RGPackageStatus::pkgStatus.getUpRisk(pkg);
          g_value_set_object(value, risk);
          break;
       }
      case DESCR_COLUMN:
...
static void gtk_pkg_list_sort(GtkPkgList *pkg_list)
{
   ...
   case RISK_COLUMN:
      if(pkg_list->order)
	 pkg_list->_lister->sortPackages(RPackageLister::LIST_SORT_RISK_ASC);
      else
	 pkg_list->_lister->sortPackages(RPackageLister::LIST_SORT_RISK_DES);
      break;
   case CURR_RISK_COLUMN:
      if(pkg_list->order)
	 pkg_list->_lister->sortPackages(RPackageLister::LIST_SORT_CURR_RISK_ASC);
      else
	 pkg_list->_lister->sortPackages(RPackageLister::LIST_SORT_CURR_RISK_DES);
      break;
   case UP_RISK_COLUMN:
      if(pkg_list->order)
	 pkg_list->_lister->sortPackages(RPackageLister::LIST_SORT_RISK_CHANGE_ASC);
      else
	 pkg_list->_lister->sortPackages(RPackageLister::LIST_SORT_RISK_CHANGE_DES);
      break;
   default:
