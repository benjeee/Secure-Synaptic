...
void setupTreeView(GtkWidget *treeview)
{
  ...
  gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(treeview),TRUE);
   ...
   /* Risk Score */
   pos = _config->FindI("Synaptic::riskColumnPos", 8);
   visible = _config->FindI("Synaptic::riskColumnVisible", true);
   if(visible){
     renderer = gtk_cell_renderer_pixbuf_new();
     column = gtk_tree_view_column_new_with_attributes(_("Risk Score"),
                                                     renderer,
                                                     "pixbuf", RISK_COLUMN,
                                                     NULL);
     gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_FIXED);
     gtk_tree_view_column_set_fixed_width(column, 72);
     //gtk_tree_view_insert_column(GTK_TREE_VIEW(treeview), column, pos);
     gtk_tree_view_column_set_sort_column_id(column, RISK_COLUMN);
     all_columns.push_back(std::pair<int, GtkTreeViewColumn *>(pos, column));
   }
   
   pos = _config->FindI("Synaptic::currRiskColumnPos", 9);
   visible = _config->FindI("Synaptic::currRiskColumnVisible", true);
   if(visible){
     renderer = gtk_cell_renderer_pixbuf_new();
     column = gtk_tree_view_column_new_with_attributes(_("Installed Risk"),
                                                     renderer,
                                                     "pixbuf", CURR_RISK_COLUMN,
                                                     NULL);
     gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_FIXED);
     gtk_tree_view_column_set_fixed_width(column, 72);
     //gtk_tree_view_insert_column(GTK_TREE_VIEW(treeview), column, pos);
     gtk_tree_view_column_set_sort_column_id(column, CURR_RISK_COLUMN);
     all_columns.push_back(std::pair<int, GtkTreeViewColumn *>(pos, column));
   }
   
   pos = _config->FindI("Synaptic::upRiskColumnPos", 10);
   visible = _config->FindI("Synaptic::upRiskColumnVisible", true);
   if(visible){
     renderer = gtk_cell_renderer_pixbuf_new();
     column = gtk_tree_view_column_new_with_attributes(_("Change in Risk"),
                                                     renderer,
                                                     "pixbuf", UP_RISK_COLUMN,
                                                     NULL);
     gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_FIXED);
     gtk_tree_view_column_set_fixed_width(column, 72);
     //gtk_tree_view_insert_column(GTK_TREE_VIEW(treeview), column, pos);
     gtk_tree_view_column_set_sort_column_id(column, UP_RISK_COLUMN);
     all_columns.push_back(std::pair<int, GtkTreeViewColumn *>(pos, column));
   }
   

   /* Description */
   pos = _config->FindI("Synaptic::descrColumnPos", 11);
